...
def step_rk4(species, reactions, h, T, rho):
    k1 = np.array([ sum([ reaction.step(specie.name, T, rho, species) for reaction in specie.reactions ]) for specie in species ])
    ...
    k2 = np.array([ sum([ reaction.step(specie.name, T + h/2, rho2, species2) for reaction in specie.reactions ]) for specie in species2 ])
    
    rho3 = rho
    species3 = [ species2[i].create_intermediate(k2[i]/2, h) for i in range(len(species2)) ]
    k3 = np.array([ sum([ reaction.step(specie.name, T + h/2, rho3, species3) for reaction in specie.reactions ]) for specie in species3 ])
    
    rho4 = rho
    species4 = [ species3[i].create_intermediate(k3[i], h) for i in range(len(species3)) ]
    k4 = np.array([ sum([ reaction.step(specie.name, T + h, rho4, species4) for reaction in specie.reactions ]) for specie in species4 ])
